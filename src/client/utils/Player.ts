...
export default class Player {
  ...
  assetsPath: any;
  animations: any;
  pathLines: THREE.Object3D<THREE.Event>;
  ...
  constructor(options: any) {
    ...
    options.app.xrScene.Scene.add(options.object);

    this.object = options.object;
    this.pathLines = new THREE.Object3D();
    this.pathColor = new THREE.Color(0xffffff);
    options.app.xrScene.Scene.add(this.pathLines);
    ...
    if (options.anims) {
      ...
      options.anims.forEach(function (anim: any) {
        self.animations[anim.name] = THREE.AnimationUtils.subclip(
          clip,
          anim.name,
          anim.start,
          anim.end
  ...
  showPathLines() {
    ...
    // Draw debug lines
    this.calculatedPath.forEach(function (vertex: any) {
  ...
  set action(name: string) {
    ...
    if (clip !== undefined && this.mixer != null) {
